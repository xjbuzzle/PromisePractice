...
function Promise(executor){
    ...
    function resolve(data){
        ...
        //状态改完之后，要执行回调函数，必须是异步执行回调，所以加个定时器，让它进队列
        setTimeout(()=>{
...
Promise.prototype.then = function(onResolved,onRejected){
    ...
    return new Promise((resolve,reject)=>{
        ...
        if(self.promiseState === 'resolved' || self.promiseState === 'fullfilled'){
            ...
            //由于then方法里面指定的回调函数是异步执行的，所以需要用定时器包裹,让它进队列
            setTimeout(()=>{
                callback(onResolved);
            });
        }
...
//resolve()方法是Promise函数的方法，不是Promise实例化对象的方法
Promise.resolve = function(value){
    //resolve方法接收一个参数，返回一个成功/失败的promise对象,对象由value决定
    return new Promise((resolve,reject)=>{
...
//reject()方法是Promise函数的方法，不是Promise实例化对象的方法
Promise.reject = function(reason){
    //reject()方法不管接收什么参数，都是返回一个失败的promise对象，值就是那个reason
    return new Promise((resolve,reject)=>{
        reject(reason);
    })
}
